...
Есть серия недорогих 51 контроллеров от производителя WCH известного своим USB to Serial ch340, и они отличаются от других наличием встроенной поддержки USB. В папке extra можно найти программатор под windows (можете поискать более свежий на сайте производителя), и английский даташит на ch554. Увы все остальные даташиты на китайском.
...
Останавлюсь подробнее на ch552/ch554 они на мой взгляд наиболее интересны, все что ниже будет касаться обеих. Они отличаются только тем, что 554 умеет быть хостом, а 552 - нет.

- 16 кБайт FLASH, из них 2 кБайта можно выделить под загрузчик и в них входит 128 байт псевдо EEPROM

- 1280 байт RAM, из них 256 байт стандартная память 51, а 1024 байта - типа внешняя

- тактовая частота до 24 МГц (можно без внешнего кварца)

- 13 ног для использования (считая Reset который можно назначить просто портом)

- на 4х ногах ADC, слабенький, всего [0-256]

- на 2х ногах PWM, тоже не блещет, [0-256], у PWM собственный таймер

- 3 стандартных x51 таймера T0,T1,T2. T2 имеет пару входов для захвата

- 2 UART, один стандартный x51, второй без 9-битного режима но с собственным генератором

- вроде как есть SPI, правда я так и не смог его запустить, и нет i2c

- питается от 5 вольт - при этом использует встроенный преобразователь 5->3.3 . Но можно питать и в обход его от 3.3В. Разумеется выводы толерантны к 5В.

- USB 2.0 full speed, с памятью контроллера общается через DMA. Максимальная длина пакета - 64 байта. Что то подсказывает мне что косячки имеются, например не отображаются на компьютере дескриптор производителя и версии, но сама передача работает со свистом.

- заявлены 6 ног со встроенной поддержкой Touch-key - еще не пробовал

- прерывания, вотч-дог, и так далее

- минимальная обвеска при подключении: кондансатор 0.47 мкФ с ноги V33 на землю.

Самое прикольное что им не нужен внешний программатор, они программируются прямо по USB. Для этого необходимо притянуть P3.6/UDP через резистор 10к ноге V33 (это **выход** питания!). В программаторе есть возможность перезапуска контроллера после прошивки, однако если вы используете USB CDC оно не срабатывает (не проходит ресет USB) и приходится после прошивки физически переподключать устройство.

## Реализация

Библиотека подразумевает работу контроллера от встроенного генератора без внешнего кварца с частотой 24 MHz. Параметры памяти и ноги настроены на **ch552** или **ch554**. Постоянно существует буфер USB CDC отъедая почти четверть имеющегося XRAM.
...
`PWM` - псевдо-аналоговый выход PWM
...
`word analogRead(byte PIN)` - прочитать аналоговое значение с ADC. Естественно это возможно только на тех ногах на которых он доступен: `D11, D14, D15, D32`. Необходимо предварительно использовать `pinMode(Dxx,ADC)`. Диапазон преобразования [0...Vcc] -> `[0..255]`.

`void analogWrite(byte PIN, word value)` - установить значение PWM. Допустимо только для тех ног которые поддерживают PWM: `D14, D30, D34, D31`. Важно: число каналов PWM меньше чем число выводов, некоторые выводы нельзя использовать для PWM одновременно: D14 и D30, D34 и D31. Для инициализации необходимо использовать `pinMode(Dxx, PWM)`. Частота PWM установлена примерно в `2кГц`, диапазон уровней `[0..255]` -> [0..Vcc].
...
Хардварного I2C у данного контроллера нет. Возможно потом сделаю софтварный.

### SPI

Пока не разобрался. Даже тестовые примеры от производителя вызывают постоянную перезагрузку процессора и ничего не делают. Делаю как в написано в даташите - перезагрузки нет, но тоже тишина.

### EEPROM

EEPROM есть, но слабоват - всего 10000 перезаписей (для каждого из байтов). Размер EEPROM - 128 байт. Доступны команды:

`byte EEPROMRead(byte address)` - чтение байта из EEPROM

`void EEPROMWrite(byte address, byte c)` - запись байта в EEPROM, причем это полная копия `EEPROMUpdate(byte address, byte c)`.  Обе команды пишут байт только в случае если он изменился.
...
void loop() {
    analogWrite(D14,analogRead(D11));
    delay(10);
}
```
